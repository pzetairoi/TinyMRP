...
log "[7/10] Configure mongod (no-auth, simple)…"
# Bind only to localhost by default (safer with no auth)
...
# Ensure authorization disabled (remove/override any previous setting)
if grep -q 'authorization:\s*enabled' /etc/mongod.conf 2>/dev/null; then
  sed -i 's/authorization:\s*enabled/# authorization: enabled (disabled by installer)/' /etc/mongod.conf || true
fi
# Make sure "security:" block does not force auth
if ! grep -q '^\s*security:' /etc/mongod.conf; then
  echo -e "\nsecurity:\n  # authorization disabled for simplicity\n" >> /etc/mongod.conf
fi
...
# Create DB once (no users needed). This is idempotent.
...
# Use repo configs if present; patch Deliverables path; else write fallback
...
# Use repo unit if present; else create robust wrapper service
...
if [[ -f "${TINY_APP_DIR}/server_config/tinymrp_server.service" ]]; then
  ...
  # normalize paths/user and inject env file
